document.addEventListener("DOMContentLoaded", () => {
  ...
  async function fetchOrders() {
    ...
    try {
      ...
      let index = reversedOrders.length;
...
$(document).ready(function () {
  $("#sub-btn-del").click(function () {
    const searchType = $("#category").val();
    const searchValue = $("#product-id-input-del").val().toLowerCase();
    ...
    $(
      ...
      // Exact match for OrderNumber, partial match for others
      const isMatch =
  ...
  // Keep your existing reset functionality
  $("#product-id-input-del").on("input", function () {
    const searchValue = $(this).val();
    if (searchValue === "") {
      $("#orders-table-body tr").show();
